...
Pipe::Pipe(const net::GraphLink& edge, EventQueue* event_queue,
           bool interesting)
    : EventConsumer(GetPipeId(edge.src(), edge.dst()), event_queue),
      delay_(event_queue->ToTime(edge.delay())),
      other_end_(nullptr) {
  graph_link_ = &edge;
  if (interesting) {
    AddMetrics(edge.src(), edge.dst());
  }
}
...
Queue::Queue(const net::GraphLink& edge, EventQueue* event_queue,
             bool interesting)
    : Queue(edge.src(), edge.dst(), event_queue, interesting) {}
...
void Queue::ApplyValue(double value) {
  uint64_t current_rate_bps = GetRateBps();
  uint64_t new_rate = static_cast<uint64_t>(value);
  if (new_rate != current_rate_bps) {
    SetRate(new_rate);
  }
}
...
void FIFOQueue::SetRate(uint64_t new_rate_bps) {
  rate_bps_ = new_rate_bps;
  CHECK(rate_bps_ > 0) << "Zero rate at queue: " << id();
  // TODO: make this independent of the event queue's clock resolution
  time_per_bit_ = EventQueueTime(pow(10, 12.0) / rate_bps_);
}

FIFOQueue::FIFOQueue(const std::string& src, const std::string& dst,
                     uint64_t rate_bps, uint64_t max_size_bytes,
                     EventQueue* event_queue, bool interesting)
...
FIFOQueue::FIFOQueue(const net::GraphLink& edge, uint64_t max_size_bytes,
                     EventQueue* event_queue, bool interesting)
    : FIFOQueue(edge.src(), edge.dst(), edge.bandwidth_bps(), max_size_bytes,
...
RandomQueue::RandomQueue(const std::string& src, const std::string& dst,
                         ...
                         EventQueue* event_queue, bool interesting)
    : FIFOQueue(src, dst, rate_bps, max_size_bytes, event_queue, interesting),
...
RandomQueue::RandomQueue(const net::GraphLink& edge, uint64_t max_size_bytes,
                         uint64_t drop_threshold_bytes, double seed,
                         EventQueue* event_queue, bool interesting)
    : RandomQueue(edge.src(), edge.dst(), edge.bandwidth_bps(), max_size_bytes,
                  drop_threshold_bytes, seed, event_queue, interesting) {}
