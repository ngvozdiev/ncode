...
class MetricsParser {
 public:
  ...
  struct WrappedEntry {
    WrappedEntry(size_t manifest_index,
        ...
        : manifest_index(manifest_index),
          manifest_entry(std::move(manifest_entry)),
          num_entries(0),
          num_non_zero_entries(0),
          sum(0) {}

    WrappedEntry(WrappedEntry&& other)
        : manifest_index(other.manifest_index),
          manifest_entry(std::move(other.manifest_entry)),
          num_entries(other.num_entries),
          num_non_zero_entries(other.num_non_zero_entries),
          sum(other.sum) {
      other.num_entries = 0;
      other.num_non_zero_entries = 0;
      other.sum = 0;
    }
  ...
  // in it.
  std::vector<WrappedEntry> ParseManifest() const;

  // Returns a string summary of all metrics in the manifest.
  std::string FullManifestToString() const;

  // Returns an html table with a summary of all metrics in the manifest. If a
  // link generator callback is provided then each metric's id in the table will
  // become a link generated by the callback.
  web::HtmlTable FullManifestToTable(
      const std::string& table_id,
      std::function<std::string(const std::string&)> link_gen =
          std::function<std::string(const std::string&)>()) const;

  // Returns a string summary of the fields of the metric whose id exactly
  // matches the argument.
  std::string ManifestToString(const std::string& metric_id) const;

  // Like ManifestToString, but populates an HTML table.
  web::HtmlTable ManifestToTable(const std::string& metric_id,
 ...
 private:
  ...
  EntriesGroupedByMetricId() const;
...
// Like above, but only matches data of type BinaryBlob and lets the user supply
// a function that will be used to convert the blobs.
template <typename T>
std::map<std::pair<std::string, std::string>,
