...
#include "../common/circular_array.h"
...
// Converts common::Distribution to PBDistribution. When serializing the type of
// the original distribution is coerced to double.
template <typename T>
PBDistribution DistributionToProtobuf(const Distribution<T>& dist) {
  PBDistribution out;
  out.set_count(dist.summary_stats().count());
  out.set_sum(dist.summary_stats().sum());
  out.set_sum_squared(dist.summary_stats().sum_squared());

  const std::vector<T>& quantiles = dist.quantiles();
  out.mutable_quantiles()->Reserve(quantiles.size());
  for (size_t i = 0; i < quantiles.size(); ++i) {
    out.add_quantiles(static_cast<double>(quantiles[i]));
  }

  const std::vector<double>& cs = dist.cumulative_fractions();
  out.mutable_cumulative_sum_fractions()->Reserve(cs.size());
  for (size_t i = 0; i < cs.size(); ++i) {
    out.add_cumulative_sum_fractions(cs[i]);
  }

  const std::vector<T>& top_n = dist.top_n();
  out.mutable_top_n()->Reserve(top_n.size());
  for (size_t i = 0; i < top_n.size(); ++i) {
    out.add_top_n(top_n[i]);
  }

  return out;
}

// Converts PBDsitribution to common::Distribution<double>.
...
template <typename T>
void SaveEntryToProtobuf(const Entry<Distribution<T>>& entry,
                         PBMetricEntry* out) {
  out->set_timestamp(entry.timestamp);
  const Distribution<T> entry_value = entry.value;
  *out->mutable_distribution_value() = DistributionToProtobuf(entry_value);
}
...
class MetricHandle : public MetricHandleBase,
 ...
 public:
  ...
  // Explicitly adds a value to the metric
...
template <typename T>
void PopulateManifestEntryType(PBManifestEntry* out, Distribution<T>* dummy) {
  Unused(dummy);
  out->set_type(PBManifestEntry::DISTRIBUTION);
}
